...
class SampleStats:
    ...
    def add(self, sample):
        # utt_id: x8AKssADebQ_000064150_000087750_es_asr
        fields = sample['utt_id'].split('_')
        duration = (float(fields[-3]) - float(fields[-4])) / 1000
        self.durations.append(duration)
        self.langs.append(norm_lang(sample['lang']))
        self.text_langs.append(norm_lang(sample['text_pred']))
        self.prevtext_langs.append(norm_lang(sample['prev_text_pred']))    # can be "<na>"
        self.speech_langs.append(norm_lang(sample['speech_pred']))
...
in_file = "data/lid.jsonl"
out_file = "data/lid_stats.csv"
# in_file = "logdir/lid/lid.1000"
# out_file = "tmp.txt"
